...
@implementation ObjViewController (Sensor)
...
{
    _objappStatus.sensorStatus = ObjAppStatus::SensorStatusNeedsUserToConnect;
...
- (void)sensorBatteryNeedsCharging
{
    ...
    _objappStatus.sensorStatus = ObjAppStatus::SensorStatusNeedsUserToCharge;
    [self updateAppStatusMessage];
}
...
{
    NSLog(@"connectToStructureSensorAndStartStreaming");
    ...
    if (result == STSensorControllerInitStatusSuccess || result == STSensorControllerInitStatusAlreadyInitialized)
    {
        ...
        if (_useColorCamera)
        {
            ...
            if (!_objslamState.initialized) // If we already did a scan, keep the current setting.
        ...
        if (!_useColorCamera)
            _objoptions.useHardwareRegisteredDepth = false;
        ...
        _objappStatus.sensorStatus = ObjAppStatus::SensorStatusOk;
        [self updateAppStatusMessage];
        
        // Start streaming depth data.
        NSLog(@"startStructureSensorStreaming");
...
- (void)startStructureSensorStreaming
{
    ...
    if (_useColorCamera)
    {
        ...
        _structureStreamConfig = _objoptions.useHardwareRegisteredDepth ? STStreamConfigRegisteredDepth320x240 : STStreamConfigDepth320x240;
        
        if (_objoptions.useHardwareRegisteredDepth)
        {
            ...
            optionsAreValid = [_sensorController startStreamingWithOptions:@{kSTStreamConfigKey : @(_structureStreamConfig),
                                                                             ...
                                                                             kSTColorCameraFixedLensPositionKey: @(_objoptions.lensPosition)}
                                                                     error:&error];
        }
        else
        {
                                ...
                                andColorFrame:(STColorFrame*)colorFrame
{
    if (_objslamState.initialized)
    {
        NSLog(@"sensorDidOutputSynchronizedDepthFrame");
