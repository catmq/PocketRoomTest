...
#pragma mark - ScanViewController Setup
...
- (void)setupMeshViewController
{
    ...
    NSLog(@"setupMeshViewController");
    
    _meshViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"MeshViewController"];
    ...
    if(_meshViewController)
    {
        NSLog(@"got meshviewcontroller and  initial");
        
        [_meshViewController initMeshView];
    }
    if(_meshViewNavigationController)
...
{
    NSLog(@"colorizeMeshInBackground");
...
{
    NSLog(@"presentMeshViewer");
    //[(EAGLView *)_meshViewController.view setContext:_display.context];
    [(EAGLView *)_meshViewController.modelview setContext:_display.context];
