...
#import "ObjViewpointController.h"
...
@interface ObjMeshViewController ()
{
    STMesh *_mesh;
    CADisplayLink *_displayLink;
    MeshRenderer *_renderer;
    ObjViewpointController *_objviewpointController;
...
@implementation ObjMeshViewController
...
- (void)viewDidLoad
{
    ...
    _objviewpointController = new ObjViewpointController(self.view.frame.size.width,
...
{
    _objviewpointController->setCameraProjection(projection);
...
{
    _objviewpointController->reset();
    _objviewpointController->setMeshCenter(center);
    _modelViewMatrixBeforeUserInteractions = _objviewpointController->currentGLModelViewMatrix();
}
...
- (void)draw
{
    ...
    bool viewpointChanged = _objviewpointController->update();
    
    // If nothing changed, do not waste time and resources rendering.
    if (!_needsDisplay && !viewpointChanged)
        return;
    
    GLKMatrix4 currentModelView = _objviewpointController->currentGLModelViewMatrix();
    GLKMatrix4 currentProjection = _objviewpointController->currentGLProjectionMatrix();
...
{
    // Forward to the ObjViewpointController.
    if ([gestureRecognizer state] == UIGestureRecognizerStateBegan)
        _objviewpointController->onPinchGestureBegan([gestureRecognizer scale]);
    else if ( [gestureRecognizer state] == UIGestureRecognizerStateChanged)
        _objviewpointController->onPinchGestureChanged([gestureRecognizer scale]);
}
...
- (void)oneFingerPanGesture:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    if([gestureRecognizer state] == UIGestureRecognizerStateBegan)
        _objviewpointController->onOneFingerPanBegan(touchPosVec);
    else if([gestureRecognizer state] == UIGestureRecognizerStateChanged)
        _objviewpointController->onOneFingerPanChanged(touchPosVec);
    else if([gestureRecognizer state] == UIGestureRecognizerStateEnded)
        _objviewpointController->onOneFingerPanEnded (touchVelVec);
}
...
- (void)twoFingersPanGesture:(UIPanGestureRecognizer *)gestureRecognizer
{
    ...
    if([gestureRecognizer state] == UIGestureRecognizerStateBegan)
        _objviewpointController->onTwoFingersPanBegan(touchPosVec);
    else if([gestureRecognizer state] == UIGestureRecognizerStateChanged)
        _objviewpointController->onTwoFingersPanChanged(touchPosVec);
    else if([gestureRecognizer state] == UIGestureRecognizerStateEnded)
        _objviewpointController->onTwoFingersPanEnded (touchVelVec);
}

- (void)touchesBegan:(NSSet *)touches
           withEvent:(UIEvent *)event
{
    _objviewpointController->onTouchBegan();
}
