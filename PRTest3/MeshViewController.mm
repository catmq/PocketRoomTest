...
@property UIDocumentInteractionController* documentInteractionController; // for save obj
               ...
               bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        */
        ...
        // for saving obj
        UIBarButtonItem *saveButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self  action:@selector(enterMeshname)];
        ...
        self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects: emailButton, saveButton, nil];
        //
        ...
        // Initialize Joystick.
        const float joystickFrameSize = self.modelview.frame.size.height * 0.4;
        CGRect joystickFrame = CGRectMake(0, self.modelview.frame.size.height-joystickFrameSize,joystickFrameSize,joystickFrameSize);
        ...
        [self.modelview addSubview:_translationJoystick.view];
        ...
        [self.modelview addSubview:_rulerText];
        [self.modelview sendSubviewToBack:_rulerText];
        ...
        [self.modelview addSubview:_circle1];
        [self.modelview addSubview:_circle2];
        [self.modelview sendSubviewToBack:_circle1];
        [self.modelview sendSubviewToBack:_circle2];
...
- (void) initMeshView
{
    ...
    */
    ...
    self.navigationController.navigationBar.backgroundColor = [UIColor colorWithRed:0.5f green:0.5f blue:0.5f alpha:0.8f];
...
- (void) enterMeshname
{
    ///
    NSLog(@"enterMeshname");
    UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"Save 3D Model"
    ...
    UIAlertAction* defaultAction = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction * action) {
                                                              ...
                                                              // objname = ((UITextField *)[alert.textFields //objectAtIndex:0]).text;
                                                              NSLog(@"get name and save mesh now!");
                                                              NSString * filename = [NSString stringWithFormat: @"%@.obj", alert.textFields.firstObject.text];
                                                              [self saveMesh:filename];
    ...
    [alert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textobjname) {
...
- (void) saveMesh:(NSString *)filename
{
    ...
    //
    // check and create folder for app
    ///
    
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0]; // Get documents folder
    NSString *appDataDirectoryPath = [documentsDirectory stringByAppendingPathComponent:@"/ModelFolder"];
    
    
    if (![[NSFileManager defaultManager] fileExistsAtPath:appDataDirectoryPath])
        [[NSFileManager defaultManager] createDirectoryAtPath:appDataDirectoryPath withIntermediateDirectories:NO attributes:nil error:&error]; //Create folder
    
    
    NSLog(@"=====> the folder is: %@", appDataDirectoryPath);
    //
    //
    
    
    NSString *filePath = [appDataDirectoryPath stringByAppendingPathComponent:filename];
    
    
    bool bSaveFile = false;
    bSaveFile = [self.meshRef writeToFile:filePath
    ...
    if(bSaveFile)
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Success!"
                                                        message:@"the model file is already saved in the device"
                                                       delegate:nil
    ...
    //get file path for other app to open
    NSURL* fileURL = [NSURL fileURLWithPath:filePath];
    if(self.documentInteractionController == nil)
    {
        self.documentInteractionController = [UIDocumentInteractionController interactionControllerWithURL:fileURL];
        NSLog(@" set to documentInteractionController == %@", fileURL);
    }
    else
    {
    ...
    bool bAppopen = [self.documentInteractionController presentOpenInMenuFromBarButtonItem:self.navigationItem.rightBarButtonItems.lastObject animated:true];
    
    if(!bAppopen)
    {
        NSLog(@"No app can open it (.obj)");
...
- (void)viewDidLoad
{
    ...
    _viewpointController = new ViewpointController(self.modelview.frame.size.width, self.modelview.frame.size.height);
...
- (void)setupGestureRecognizer
{
    ...
    [self.modelview addGestureRecognizer:pinchScaleGesture];
    ...
    [self.modelview addGestureRecognizer:rotationGesture];
    ...
    [self.modelview addGestureRecognizer:singleTapGesture];
}
...
- (void)dismissView
{
    ...
    [(EAGLView *)self.modelview setContext:nil];
...
- (void)draw
{
    ...
    //NSLog(@"draw: %d"+self.modelview);
    
    [(EAGLView *)self.modelview setFramebuffer];
    ...
    [(EAGLView *)self.modelview presentFramebuffer];
...
{
    float scale = self.modelOpacitySlider.value;
    
    self.modelview.alpha = scale;
...
-(GLKVector3)screenPointTo3dPoint :(GLKVector2)screenPt
{
    ...
    // scale to [-1 1]
    GLKVector2 screenPtScale = GLKVector2Make(2*screenPt.x/self.modelview.frame.size.width-1.0, 2*screenPt.y/self.modelview.frame.size.height-1.0);
...
- (bool)point3dToScreenPoint:(GLKVector3)pt screenPt:(GLKVector2&)screenPt
{
    ...
    float height = self.modelview.frame.size.height;
    ...
    bool inView =  ptTransformed.z > 0 && [self.modelview pointInside:CGPointMake(screenPt.x, screenPt.y) withEvent:nil];
    return inView;
...
- (void)singleTapGesture:(UITapGestureRecognizer *)gestureRecognizer
{
    ...
    {
        CGPoint touchPoint = [gestureRecognizer locationInView:self.modelview];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    CGPoint touchPoint = [touch locationInView:self.modelview];
