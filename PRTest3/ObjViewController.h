...
enum ObjScannerState
{
    // Defining the volume to scan
    ObjScannerStateCubePlacement = 0,
    
    // Scanning
    ObjScannerStateScanning,
    
    // Visualizing the mesh
    ObjScannerStateViewing,
...
struct ObjSlamData
{
    ObjSlamData ()
    : initialized (false)
    , objscannerState (ObjScannerStateCubePlacement)
    {}
    ...
    ObjScannerState objscannerState;
};
...
struct ObjDisplayData
{
    ...
    ///
    std::vector<uint8_t> cameraviewBuffer;
    GLuint cameraviewTexture;
    GLuint cameraFrameBuffer;
    
    std::vector<uint8_t> meshviewBuffer;
    GLuint meshviewTexture;
    GLuint meshFrameBuffer;
    
    std::vector<uint8_t> combineviewBuffer;
    GLuint combineviewTexture = -1;
};

@interface ObjViewController : UIViewController <STBackgroundTaskDelegate, ObjMeshViewDelegate, UIPopoverControllerDelegate, UIGestureRecognizerDelegate>
{
    // Structure Sensor controller.
    STSensorController *_sensorController;
    STStreamConfig _structureStreamConfig;
    
    ObjSlamData _objslamState;
    
    ObjOptions _objoptions;
    
    // Manages the app status messages.
    ObjAppStatus _objappStatus;
    
    ObjDisplayData _objdisplay;
    ...
    UINavigationController *_objmeshViewNavigationController;
    ObjMeshViewController *_objmeshViewController;
...
@property (nonatomic) LoadViewController *LVcontroller;
