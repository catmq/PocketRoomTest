...
void MeshRenderer::clear()
{
    ...
    if(d->currentRenderingMode == RenderingModeXRay)
        glClearColor(0.1, 0.1, 0.1, 1.0);             // for x ray
    else
        glClearColor(0.0, 0.0, 0.0, 0.0);             // background for mesh view
...
void MeshRenderer::uploadMesh (STMesh* mesh)
{
    ...
    {
        NSLog(@"d->hasPerVertexColor: yes");
    }
    if (d->hasPerVertexNormals)
    {
        NSLog(@"d->hasPerVertexNormals: yes");
    }
    if (d->hasPerVertexUV)
    {
        NSLog(@"d->hasPerVertexUV: yes");
    }
    if (d->hasTexture)
    {
        NSLog(@"d->hasTexture: yes");
        uploadTexture ([mesh meshYCbCrTexture]);
    }
    
    NSLog(@"numUploads: %d",numUploads);
...
{
    NSLog(@"MeshRenderer::uploadTexture");
    ...
    NSLog(@"Texture w h: %d, %d",width,height);
